...
static long hispcpe_ioctl(struct file *filp, unsigned int cmd, unsigned long args)
{
    ...
    switch (cmd) {
        case HISP_CPE_PWRUP:
            ...
            if (!wake_lock_active(&dev->ipp_wakelock)) {
                wake_lock(&dev->ipp_wakelock);
                pr_info("ipp power up wake lock.\n");
            }
            mutex_unlock(&dev->ipp_wakelock_mutex);/*lint !e456 */
...
static int hispcpe_probe(struct platform_device *pdev)
{
    ...
    atomic_set(&dev->cpe_slam_refs, 0);
    mutex_init(&dev->lock);
    wake_lock_init(&dev->ipp_wakelock, WAKE_LOCK_SUSPEND, "ipp_wakelock");
...
static int hispcpe_remove(struct platform_device *pdev)
{
    ...
    atomic_set(&dev->cpe_mfnr_refs, 0);
    atomic_set(&dev->cpe_slam_refs, 0);
    wake_lock_destroy(&dev->ipp_wakelock);
